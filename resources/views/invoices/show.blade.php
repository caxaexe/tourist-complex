        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 shadow rounded p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <div class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">Клиент</div>
                        <div class="text-lg font-semibold text-gray-800 dark:text-gray-200 dark:text-gray-200">
                            {{ $invoice->booking->client->full_name ?? '—' }}
                        </div>

                        <div class="mt-2 text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">Номер / Комната</div>
                        <div class="text-gray-800 dark:text-gray-200 dark:text-gray-200">
                            №{{ $invoice->booking->room->number ?? '—' }}
                            @if(!empty($invoice->booking->room->roomType))
                                <span class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">
                    ...
                    <div class="sm:text-right">
                        <div class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">Дата выставления</div>
                        <div class="text-gray-800 dark:text-gray-200 dark:text-gray-200">
                            {{ optional($invoice->issued_at)->format('d.m.Y') ?? '—' }}
                        </div>

                        <div class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400 mt-2">Оплатить до</div>
                        <div class="text-gray-800 dark:text-gray-200 dark:text-gray-200">
                            {{ optional($invoice->due_at)->format('d.m.Y') ?? '—' }}
                        </div>

                        <div class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400 mt-2">Статус счёта</div>
                        <div class="text-gray-800 dark:text-gray-200 dark:text-gray-200">
                            {{ $invoice->status }}
                        </div>
                    </div>
                ...
                <hr class="my-6 border-gray-200 dark:border-gray-700 dark:border-gray-700">

                {{-- Позиции счета --}}
                <div class="text-lg font-semibold text-gray-800 dark:text-gray-200 dark:text-gray-200 mb-3">
                    Позиции
                </div>
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="text-left border-b border-gray-200 dark:border-gray-700 dark:border-gray-700">
                                <th class="py-2 text-gray-700 dark:text-gray-200 dark:text-gray-200">Позиция</th>
                                <th class="text-right text-gray-700 dark:text-gray-200 dark:text-gray-200">Кол-во</th>
                                <th class="text-right text-gray-700 dark:text-gray-200 dark:text-gray-200">Цена</th>
                                <th class="text-right text-gray-700 dark:text-gray-200 dark:text-gray-200">Сумма</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($invoice->items as $item)
                                <tr class="border-b border-gray-200 dark:border-gray-700 dark:border-gray-700">
                                    ...
                                    <td class="text-right text-gray-800 dark:text-gray-200 dark:text-gray-200">
                                        {{ $item->quantity }}
                                    </td>
                                    <td class="text-right text-gray-800 dark:text-gray-200 dark:text-gray-200">
                ...
                <div class="mt-4 text-right">
                    <div class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">Итого по счёту</div>
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div>
                        ...
                        <div class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">
                            Оплачено: <b class="text-gray-800 dark:text-gray-200 dark:text-gray-200">{{ number_format($paid,2,'.',' ') }}</b>
                            • Остаток: <b class="text-gray-800 dark:text-gray-200 dark:text-gray-200">{{ number_format($balance,2,'.',' ') }}</b>
                        </div>
                    </div>
                ...
                <div class="mt-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 dark:border-gray-700 rounded p-4 overflow-auto">
                    <table class="w-full">
                        <thead>
                            <tr class="text-left border-b border-gray-200 dark:border-gray-700 dark:border-gray-700">
                                ...
                                <th class="text-gray-700 dark:text-gray-200 dark:text-gray-200">Комментарий</th>
                        ...
                        <tbody>
                            @forelse($invoice->payments as $p)
                                <tr class="border-b border-gray-200 dark:border-gray-700 dark:border-gray-700">
                                    <td class="py-2 text-gray-800 dark:text-gray-200 dark:text-gray-200">{{ $p->id }}</td>
                                    <td class="text-gray-800 dark:text-gray-200 dark:text-gray-200">{{ number_format((float)$p->amount,2,'.',' ') }}</td>
                                    <td class="text-gray-800 dark:text-gray-200 dark:text-gray-200">{{ $p->method }}</td>
                                    <td class="text-gray-800 dark:text-gray-200 dark:text-gray-200">{{ optional($p->paid_at)->format('d.m.Y H:i') }}</td>
                                    <td class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400">{{ $p->note ?? '—' }}</td>
