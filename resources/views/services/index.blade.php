                ...
                <table class="w-full">
                    <thead>
                        <tr class="text-left border-b border-gray-200 dark:border-gray-700">
                            ...
                            <th class="text-gray-700 dark:text-gray-200">{{ __('Цена') }}</th>
                    ...
                    <tbody>
                        @forelse($services as $service)
                            <tr class="border-b border-gray-200 dark:border-gray-700">
                                <td class="py-2 text-gray-800 dark:text-gray-200">{{ $service->id }}</td>
                                <td class="text-gray-800 dark:text-gray-200">{{ $service->name }}</td>
                                <td class="text-gray-800 dark:text-gray-200">{{ number_format($service->price, 2, '.', ' ') }}</td>
                                <td class="text-right whitespace-nowrap">
                                    <a href="{{ route($prefix.'services.edit', $service) }}"
                                       class="inline-block px-3 py-1 rounded border border-gray-200 dark:border-gray-700 text-blue-700 dark:text-blue-300 hover:bg-gray-50 dark:hover:bg-gray-700/30">
                                        {{ __('Редактировать') }}
                                    </a>
                                    ...
                                    <form class="inline"
                                          ...
                                          onsubmit="return confirm('{{ __('Удалить услугу?') }}')">
                                        @csrf
                                        @method('DELETE')
